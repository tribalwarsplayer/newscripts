...
(function() {
    ...
    if (!TwCheese.hasTool(toolId)) {
        ...
        function scrapePageNumber() {
            ...
            if (currentPage && !currentPage.includes('összes')) {
        ...
        class HaulStatsWidget_HaulStatsWidget extends AbstractWidget["a" /* AbstractWidget */] {
            ...
            createHtml() {
                ...
                let pageInfo = this.pageNumber ? `az ${this.pageNumber}. oldalrĂłl` : '';
                    ...
                    <div id="twcheese_pillaging_stats" class="vis widget">
                        <h4>
                            ...
                            <span style="font-size: 10px; font-style: normal; font-weight: normal; margin-right: 25px; float: right;">
                                kĂŠszĂ­tette: <a target="_blank" href="https://forum.tribalwars.net/index.php?members/28484/">cheesasaurus</a>
                            </span>
                        </h4>
                        <div id="twcheese_pillaging_stats_content" style="display: ${contentDisplay};">
                            ...
                            <div class="twcheese-pillaging-stats-summation">
                                <div style="text-align: center; width: 100%; margin-top: 5px; margin-bottom: 5px;">
                                     <select id="twcheese_pillaging_stats_from">${summationFromOptions.join('')}</select>-tĂłl
                            ...
                            <table class="twcheese-pillaging-stats-hourly-breakdown" width="100%">
                                <tbody>
                                    <tr><td colspan="6" style="text-align: center; font-size: 16px;">BeĂŠrkezĹ nyersanyagok ${pageInfo}</td></tr>
                                    <tr>
                                        <th>ĂrkezĂŠs</th>
                                        ...
                                        <th colspan="2">TeljesĂ­tmĂŠny</th>
                                    </tr>
                ...
                <div style="height: 100%; width: 100%; background: url('${ImageSrc["a" /* ImageSrc */].popupBackground}')">
                    <div style="background: no-repeat url('${ImageSrc["a" /* ImageSrc */].servant}');">
                        <div id="twcheese_servant_text">
                            <p style="font-size: 16px;">ĂdvĂśzĂśllek,</p>
                            <p>SzeretnĂŠd a fosztogatĂĄsokat megjelenĂ­teni?</p>
                        </div>
                                ...
                                <tbody>
                                    <tr>
                                        <td id="twcheese_servant_info_prompt" class="twcheese-servant-info">
                                            <h5>FarmolĂĄsi informĂĄciĂłk betĂśltĂŠse?</h5>
                                        ...
                                        <td id="twcheese_servant_info_loading" class="twcheese-servant-info" style="display: none;">
                                            <h5>FosztogatĂĄsok betĂśltĂŠse</h5>
        ...
        function buildContent(message, options) {
            ...
            if (options.skippableId) {
                html += `<div class="twcheese-suggest-redirect-skip-container">
                    ...
                    <span>LegkĂśzelebb ne kĂŠrdezd, hanem egybĹl vigyĂŠl oda.</span>
                </div>`
            }
            return html;
        }
        ...
        async function alertPremiumRequired() {
            let message = `
                Ez a script olyan funkciĂłkra tĂĄmaszkodik, amelyek csak a <a href="game.php?screen=premium">prĂŠmium</a> felhasznĂĄlĂłk szĂĄmĂĄra ĂŠrhetĹek el.
                <br/>
                <p style="font-size: 10px;">
                    $999-ĂŠrt monitorĂĄllvĂĄnyt vĂĄsĂĄrolhatnĂĄl
                    <br/>Vagy <strong>30+ ĂŠv</strong> kh prĂŠmium fiĂłkot!
                </p>`;
        
            let buttonText = `Ăgy nĂŠzek ki, mint aki pĂŠnzt keres?`;
        ...
        function parseArrivalHungarian(text) {
            // e.g. "mĂĄrc 31, 2020  22:13:49:000"
            let expr = /(\D+) (\d+), (\d+)  (\d+):(\d+):(\d+):?(\d+)?/;
			...
			switch (monthName) {
				...
				break;
                case 'mĂĄrc': monthName = "mar";
				break;
                case 'ĂĄpr': monthName = "apr";
				break;
                case 'mĂĄj': monthName = "may";
				break;
                case 'jĂşn': monthName = "jun";	
				break;
                case 'jĂşl': monthName = "jul";
				break;
        ...
        function suggestRedirectToCommandsOverview() {
            suggestRedirect({
                message: `
                    <p style="font-size:14px;">A hasznĂĄlatĂĄhoz a Parancsok felĂźleten kell lenned.</p>
                    <p style="font-size:12px;">VĂĄlaszd a visszatĂŠrĹ fĂźlet, hisz csak azok a csapatok hozhatnak magukkal nyersanyagot. :)</p>`,
        ...
        function useTool() {
            ...
            if (!document.getElementById('commands_table')) {
                window.UI.ErrorMessage(`Ăgy tĹąnik nincs egyetlen parancs sem.`, 3000);
                return;
            }
